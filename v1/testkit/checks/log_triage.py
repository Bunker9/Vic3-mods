...
Produces, per mod:
  - errors  : error.log entries whose script-location / message references THIS mod's files
              (each marked benign or real).
  - census  : for mods whose props.toml declares a [census] (a debug_log marker + expected
              caps), how many levels of each building were placed vs the expected cap.
And one shared:
  - non_mod_errors : real errors that reference NO tracked mod (vanilla / other) - reported
                     separately so they don't pollute a mod's result (your point 5).

Mod ownership of an error is decided by matching the error text against the mod's own file
paths/basenames (auto-derived by scanning the mod folder) - so it generalises to any mod.

Usage:
    python log_triage.py --logs <vic3 logs dir> --mods <mod_dir> [<mod_dir> ...] [--json out.json]
If --logs is omitted, resolves the OneDrive-redirected Documents path.
"""
import sys, os, re, json, argparse, datetime, tomllib
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from utils import resolve_logs_dir  # noqa: E402

# Universal benign substrings (engine-level, mod-agnostic). Mod-specific benign + the whole
# census config live in each mod's sanity-check/test-<Mod>/props.toml — NOT in this script.
UNIVERSAL_BENIGN = ("should be in utf8-bom encoding",)

TESTBOOK = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))  # .../testbook


def load_props(mod):
    """Read testbook/<Mod>/props.toml; return {} if absent.
    (Was testbook/test-<Mod>/props.toml — the stale sanity-check layout, which silently
    failed to load ANY props after the testbook migration: no tables/census/benign.)"""
    p = os.path.join(TESTBOOK, mod, "props.toml")
...
def parse_census(debug_lines, census_cfg):
    ...
    for row in out:
        ...
        if cap is None:
            ...
            # ADDITIVE (flavour): always build cap regardless of existing -> OK = built it.
            row["need"] = cap
            row["ok"] = row["placed"] >= cap
        else:
            # DEFICIT (champ/support): top up to cap. OK = reached/exceeded it (never demolishes,
            # so a nation already above cap is fine).
...
def run(logs_dir, mod_dirs):
    ...
    # benign = universal + any mod-specific benign substrings from props
    ...
    # census per mod that declares one in its props
    census = []
    for m in mod_names:
    ...
    # group the non-mod errors by the source file they reference
    by_file = {}
    for rec in non_mod:
    ...
    # SUSPECT-MOD: engine errors caused by OUR create_building seeding placing a building where the
    # state can't support it. Parsed into structured rows so the report names the state+building to
    # fix. KEY (user 2026-06-15): state.cpp "can only support" = a HISTORY file create_building entry
    # is WRONG for that state (load-time, no can_construct gate) -> fix/remove the history entry.
    # building_manager.cpp = runtime over-build (a pulse/effect placed past max cap). pdx_assert /
    # "Failed creating backing building" = create_building of a building that can't exist there.
    ...
    # COMPACT mod errors too: group each mod's REAL errors by source file (+count), the same
    # collapsible shape as non_mod_groups, so a noisy mod doesn't dump 100s of <pre> blocks.
    for m in mods:
        mbf = {}
