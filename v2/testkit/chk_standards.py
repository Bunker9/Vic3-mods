...
PORTED into v2 (self-contained) on 2026-06-18 from testkit/checks/standards.py.
...
Rules (deliberately ZERO-false-positive; dependency-free — no game files, CI-safe):
  MOD-ICON  every modifier definition has `icon =`
  MOD-LOC   every modifier name has a localization key
  EVT-PIC   a player-facing event (has option {}) has a `picture =`
  LOC-DASH  no '-' or '—' in player-facing .yml loc strings (dashes are debug-only)
  EFF-REMOVE bare `remove_modifier =` without a `has_modifier` guard
