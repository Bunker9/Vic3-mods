...
> Status: **BUILT — Phase 4 complete (2026-06-30); code wired + ran end-to-end clean.** VALIDATION PENDING a
> text/melted save: all current local saves are binary (ironman/compressed) so the scan yields 0 rows — feed a
> non-ironman / **melted** `.v3` (see note below) to validate. Ported the old `tools/save-game-parser/*`; the
> old `save-game-parser/` dir is retired once validated. Design home:
> `hk-config/roadmap/MOD-DEBUG-FRAMEWORK.md`. Imports
> `../Framework-common`.
...
- `--save` a specific save; else newest `*.v3` in the save-games dir (from `config-game.toml`). Handles zip
...
## Outputs — `Game-Victoria3/save-CURR-<name>/<MOD_NAME>/` (raw → data/aggr → report)
| file | class | content |
|---|---|---|
