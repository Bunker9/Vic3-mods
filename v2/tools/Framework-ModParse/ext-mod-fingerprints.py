#!/usr/bin/env python3
"""ext-mod-fingerprints.py — ModParse extractor (NEW): the mod's deliberate save-fingerprint markers ->
data-<Mod>/raw_fingerprints.csv (fp_id, fp_name, op, file_id, line_no). op = set/change/has/other inferred
from the line. Feeds Framework-SaveParse (which fp markers to look for) AND the debug-var-standardization
check (DEV-RULES: every change'd var must be set first). Infix from config (default '_fingerprint_').
...
def main(args):
    ...
    for i, (rel, ap, _fn) in enumerate(lib_io.walk_files(mod_path, set(CFG["scan"]["extensions"])), 1):
        ...
        for ln, line in enumerate(lib_io.read_text_lines(ap), 1):
            for name in dict.fromkeys(rx.findall(line)):   # unique per line, order-stable
                fp_id += 1
